...
In terms of pricing, VMware makes a public calculator available [here](https://cloud.vmware.com/vmc-aws/pricing), Microsoft does the same for the CloudSimple solution [here](https://azure.microsoft.com/en-in/pricing/details/azure-vmware-cloudsimple/). As an example for a 3 host on-demand solution for 1 month in Europe VMC on AWS would round out at about $18,000 (with no extras like stretch cluster or SRM) for 3 i3 hosts and the CloudSimple solution on Azure would come to around $14,000 for 3 CS28 hosts. Note that the VMC on AWS i3 host type has a higher core (and memory) count at 36 (there is no 28 core equivilant) so pricing wise they are close.
